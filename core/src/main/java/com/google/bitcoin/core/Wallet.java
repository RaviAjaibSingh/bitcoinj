...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
     ...
     */
    public synchronized List<ECKey> getKeys() {
     ...
     */
    public synchronized List<Script> getWatchedScripts() {
    ...
    public synchronized void saveToFile(File temp, File destFile) throws IOException {
     ...
     */
    public synchronized void setAcceptRiskyTransactions(boolean acceptRiskyTransactions) {
     ...
     */
    public synchronized boolean doesAcceptRiskyTransactions() {
     ...
     */
    public synchronized void setRiskAnalyzer(RiskAnalysis.Analyzer analyzer) {
     ...
     */
    public synchronized RiskAnalysis.Analyzer getRiskAnalyzer() {
        return riskAnalyzer;
    }
     ...
     */
    public synchronized WalletFiles autosaveToFile(File f, long delayTime, TimeUnit timeUnit,
     ...
     */
    private synchronized boolean checkForDoubleSpendAgainstPending(Transaction tx, boolean takeAction) {
    ...
    private synchronized void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
                         int relativityOffset) throws VerificationException {
     ...
     */
    private synchronized void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
     ...
     */
    private synchronized void updateForSpends(Transaction tx, boolean fromChain) throws VerificationException {
        if (fromChain)
            checkState(!pending.containsKey(tx.getHash()));
        for (TransactionInput input : tx.getInputs()) {
     ...
     */
    private synchronized void maybeMovePool(Transaction tx, String context) {
        if (tx.isEveryOwnedOutputSpent(this)) {
     ...
     */
    public synchronized Set<Transaction> getTransactions(boolean includeDead) {
     ...
     * @throws InsufficientMoneyException if the request could not be completed due to not enough balance.
     */
    public synchronized Transaction sendCoinsOffline(SendRequest request) throws InsufficientMoneyException {
        completeTx(request);
        commitTx(request.tx);
        return request.tx;
    }
     ...
     */
    public synchronized void completeTx(SendRequest req) throws InsufficientMoneyException {
     ...
     */
    public synchronized LinkedList<TransactionOutput> calculateAllSpendCandidates(boolean excludeImmatureCoinbases) {
     ...
     */
    public synchronized LinkedList<TransactionOutput> getWatchedOutputs(boolean excludeImmatureCoinbases) {
        LinkedList<TransactionOutput> candidates = Lists.newLinkedList();
        for (Transaction tx : Iterables.concat(unspent.values(), pending.values())) {
            if (excludeImmatureCoinbases && !tx.isMature()) continue;
            for (TransactionOutput output : tx.getOutputs()) {
     ...
     */
    public synchronized int addWatchedScripts(final List<Script> scripts) {
        int added = 0;
    ...
    public synchronized boolean hasKey(ECKey key) {
    ...
    public synchronized boolean isWatchedScript(Script script) {
     ...
     */
    public synchronized BigInteger getWatchedBalance(CoinSelector selector) {
	    checkNotNull(selector);
     ...
     */
    public synchronized String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
    ...
    private synchronized void toStringHelper(StringBuilder builder, Map<Sha256Hash, Transaction> transactionMap,
    ...
    @Nullable
    public synchronized Sha256Hash getLastBlockSeenHash() {
        return lastBlockSeenHash;
    }

    public synchronized void setLastBlockSeenHash(@Nullable Sha256Hash lastBlockSeenHash) {
        this.lastBlockSeenHash = lastBlockSeenHash;
    }

    public synchronized void setLastBlockSeenHeight(int lastBlockSeenHeight) {
        this.lastBlockSeenHeight = lastBlockSeenHeight;
    }

    public synchronized void setLastBlockSeenTimeSecs(long timeSecs) {
     ...
     */
    public synchronized long getLastBlockSeenTimeSecs() {
     ...
     */
    public synchronized int getLastBlockSeenHeight() {
     ...
     */
    public synchronized ECKey addNewEncryptedKey(CharSequence password) {
     ...
     */
    public synchronized void setKeyCrypter(KeyCrypter keyCrypter) {
     ...
     */
    public synchronized void setCoinSelector(@Nonnull CoinSelector coinSelector) {
     ...
     */
    public synchronized ListenableFuture<BigInteger> getBalanceFuture(final BigInteger value, final BalanceType type) {
    ...
    private synchronized void checkBalanceFuturesLocked(@Nullable BigInteger avail) {
    ...
    public synchronized Map<String, WalletExtension> getExtensions() {
    ...
    private synchronized void queueOnTransactionConfidenceChanged(final Transaction tx) {
    ...
    private synchronized void maybeQueueOnWalletChanged() {
    ...
    private synchronized void queueOnCoinsReceived(final Transaction tx, final BigInteger balance, final BigInteger newBalance) {
        for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    private synchronized void queueOnCoinsSent(final Transaction tx, final BigInteger prevBalance, final BigInteger newBalance) {
        for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    private synchronized void queueOnReorganize() {
        checkState(insideReorg);
        for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
    ...
    private synchronized void queueOnKeysAdded(final List<ECKey> keys) {
        for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    private synchronized void queueOnScriptsAdded(final List<Script> scripts) {
        for (final ListenerRegistration<WalletEventListener> registration : eventListeners) {
            registration.executor.execute(new Runnable() {
                @Override
                public void run() {
    ...
    private synchronized void maybeRotateKeys() {
